...
const Game = ({ playerName }) => {
  ...
  const setupWebSocket = (playerName, onPlayerUpdate) => {
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.send('/app/register', {}, JSON.stringify({ username: playerName, x: 0, y: 0, color: 'blue' }));
      },
      onMessage: (message) => {
  ...
  useEffect(() => {
    ...
    // Setup WebSocket
    const { sendPlayerMovement } = setupWebSocket(playerName, updatePlayerPosition);
      ...
      try {
        ...
        k.onUpdate(() => {
          ...
          if (moving) {
            ...
            sendPlayerMovement({ id: playerName, x: player.pos.x, y: player.pos.y, direction: newDirection });
